...
export default function AddVehiclePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedVehicleDetails) {
        const parsed = JSON.parse(savedVehicleDetails);
        setVehicleDetailsData(parsed);
      }
      if (savedStep) {
  ...
  // Load vehicle details from sessionStorage when navigating to vehicle details step
  useEffect(() => {
    if (!isInitialized) return;
    
    const vehicleDetailsStep = profileNeedsCompletion ? 3 : 2;
    // Always check sessionStorage when navigating to this step, even if state has data
    // This ensures we have the latest saved data (especially important after back navigation)
    if (currentStep === vehicleDetailsStep) {
      try {
        const savedVehicleDetails = sessionStorage.getItem(VEHICLE_DETAILS_KEY);
        if (savedVehicleDetails) {
          const parsed = JSON.parse(savedVehicleDetails);
          // Only update state if sessionStorage has a description and state doesn't, or if state is empty
          const shouldUpdate = !vehicleDetailsData.description || 
                               (vehicleDetailsData.description.trim().length === 0 && parsed.description && parsed.description.trim().length > 0) ||
                               Object.keys(vehicleDetailsData).length === 0;
        ...
        console.error("Failed to load vehicle details from sessionStorage:", error);
      }
    }
  }, [currentStep, isInitialized, profileNeedsCompletion]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Only save vehicleDetailsData if it has a description (don't overwrite saved data with empty state)
      if (Object.keys(vehicleDetailsData).length > 0 && vehicleDetailsData.description && vehicleDetailsData.description.trim().length > 0) {
        sessionStorage.setItem(VEHICLE_DETAILS_KEY, JSON.stringify(vehicleDetailsData));
      } else if (Object.keys(vehicleDetailsData).length > 0 && ((vehicleDetailsData.collectionIds && vehicleDetailsData.collectionIds.length > 0) || vehicleDetailsData.hourlyRate || vehicleDetailsData.dailyRate)) {
        // Save if we have other data even without description (collections, pricing)
        sessionStorage.setItem(VEHICLE_DETAILS_KEY, JSON.stringify(vehicleDetailsData));
      }
  ...
  const renderStep = () => {
    ...
    if (currentStep === vehicleDetailsStep) {
      ...
      // Read directly from sessionStorage to ensure we have the latest data (important after back navigation)
      let finalDefaultValues = vehicleDetailsData;
      try {
        const savedVehicleDetails = sessionStorage.getItem(VEHICLE_DETAILS_KEY);
        if (savedVehicleDetails) {
          const parsed = JSON.parse(savedVehicleDetails);
          // Use sessionStorage data if it has a description and state doesn't, or merge them
          if (parsed.description && parsed.description.trim().length > 0) {
            finalDefaultValues = { ...vehicleDetailsData, ...parsed };
          }
        }
      } catch (error) {
        console.error("Failed to read vehicle details from sessionStorage:", error);
      }
